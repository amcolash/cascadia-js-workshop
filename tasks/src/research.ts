...
export async function research(query: string, onEvent: (e: ResearchEvent) => void): Promise<string> {
  ...
  const results = await Promise.all(
    searches.map(async (spec, index) => {
      ...
      try {
        const started = await render.workflows.startTask(`${serviceSlug}/searchOne`, [query, spec, index]);
